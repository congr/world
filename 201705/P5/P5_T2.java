/**
 * Created by cutececil on 2017. 5. 22..
 */
...
public class P5_T2 {
    ...
    public static void main(String[] args) throws Exception {
        String inFilename = (args != null && args.length > 0) ? args[0] : "201705/P5/Set3.in"; // path from root
        ...
        while (T-- > 0) {
            ...
            LinkedHashMap<Point2D, Integer> hash = new LinkedHashMap<>();
                ...
                if (isAdj) { // 거리가 가장 먼 점을 더한다
                    ...
                    for (int j = 0; j < hulls.length; j++) {
                        if (hulls[j] != upt && hulls[j] != vpt) {
                            Point2D anyPt = hulls[j];

                            double temp = Point2D.getDistanceToSegment(upt, vpt, anyPt);
                            //if (debug) System.out.println(hulls[j] + " dist : " + temp);

                            //if (debug)
                            //    System.out.println("temp " + temp + " farDist " + farDist + " x,y [" + anyPt.x() + " " + anyPt.y());

                            if (farDist <= temp) {
                                if (debug) System.out.println("update MAX_VALUE " + farDist + " " + temp);
                                if (farDist == temp) {
                                    if (debug) System.out.println("same dist MAX_VALUE [" + farPt.x + " " + farPt.y);
                                    if (debug) System.out.println("same dist temp [" + anyPt.x + " " + anyPt.y);
                                    boolean update = false;
                                    if (farPt.x() > anyPt.x()) {
                                        update = true;
                                    } else if (farPt.x() == anyPt.x()) {
                                        if (farPt.y() > anyPt.y())
                                            update = true;
                                    }
                                    ...
                                    if (debug) System.out.println("not same just update " + farDist + " " + temp);
                                    farPt = hulls[j];
                                    farDist = temp;
                    ...
                    long number = hash.get(farPt);
                    mod += number;
                    if (debug) System.out.println("farthest " + number + " " + number + " " + farPt);
