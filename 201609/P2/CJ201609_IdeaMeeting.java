    ...
    public static void main(String[] args) throws Exception {
        ...
        while (T-- > 0) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                int y = sc.nextInt(); // 부서
                al.add(s); // for virtual v segment
                al.add(e);
                SegmentHV seg = new SegmentHV(s, y, e, y);
                pq.add(new Event(s, seg));
                pq.add(new Event(e, seg));
            }
        ...
        while (!pq.isEmpty()) {
            ...
            if (seg.isVertical()) {
                if (rangeSearchTree.size() >= target) {
                    // vertical 일때를 가정, 지금 실제 vertical 이 없으므로 항상 pq peek한 경우엔 무조건 vertical이 있다고 가정해도 된다
//                    SegmentHV seg1 = new SegmentHV(open, -INF, open, -INF); // 마지막 일치한 x지점의 무한대 vertical line
//                    SegmentHV seg2 = new SegmentHV(sweep, INF, sweep, INF); // 현재 x의 무한대 세로선
                    ...
                    if (set.size() >= target) {
                        if (sweep == seg.x1 && open == INF)
                            ...
                            allSatisfiedStart = Math.min(allSatisfiedStart, open);
                            allSatisfiedEnd = Math.max(allSatisfiedEnd, sweep);

//                            if (allSatisfiedEnd - allSatisfiedStart < sweep - open) { // 갭이 큰 것이 더 좋은 것
//                                allSatisfiedStart = open;
//                                allSatisfiedEnd = sweep;
////                                if (allSatisfiedStart != INF) allSatisfiedStart = open;
////                                if (allSatisfiedEnd < sweep) allSatisfiedEnd = sweep;
//                            } else {
//
//                            }
                        }
