...
/**
 * Created by cutececil on 2017. 9. 23..
 */
public class P2_여행 {
    public static void main(String[] args) throws Exception {
        String inFilename = (args != null && args.length > 0) ? args[0] : "201709/P2/Set3.in"; // path from root
        ...
        int bfs(int start, int R) {
            ...
            while (!pq.isEmpty()) {
                ...
                if (hereE.d == R && here == start) {
                    ...
                    //System.out.println("Rday" + R + " hereCost " + hereCost);
                }

                if (hereE.d > R) break; // 여행일이 지나면 더 갈 필요없다

                if (visited[here][hereE.d] < hereCost) continue; // !!! 더이상 가볼 필요 없다

                for (Edge thereE : adjList[here]) {
                    int there = thereE.v;
                    int thereCost = thereE.getCost(hereE.d + 1); // 매일 cost가 다르므로 다음날 어떤 cost를 쓸지 찾아온다
                    int newCost = thereCost + hereCost;
                    if (thereCost > 0 && (visited[there][hereE.d + 1] > newCost)) {
                        visited[there][hereE.d + 1] = newCost;
                        pq.add(new Edge(there, newCost, hereE.d + 1));
                    }
                }
            }
        ...
        class Edge {
            ...
            int getCost(int r) {
                if (w == null) return 0;
                    ...
                    return w[(r - 1) % w.length]; // r이 3일차라면 index는 0 1 2 로 2가 된다
            }
